...
`data/world-XX.js`，XX 为对应课次，写完在 `index.html` 加一行 `<script>`。页面顺序：带走什么 → 世界史怎么讲（叙述）→ 同一时期的世界 → 两种解读 → 三栏对照表 → 对照框。转述世俗史学的讲法，不代表课程立场；教材观点写转述大意，原文引用要注明出处。叙述各章的首段不要以数字开头（首字会做成下沉大字），测试会检查。
